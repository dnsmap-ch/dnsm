...
class DotMainLoopTest {
    ...
    private val dotMainLoop =
        DotMainLoop(stubResolverService, resultService, Formatter(), SilentOutput(::println))
    ...
    fun testRun() {
        ...
        assertDoesNotThrow { dotMainLoop.run(optionFlags) }
    }
    ...
    fun testPrepare() {
        ...
        val settings = dotMainLoop.prepare(optionFlags)

        assertThat(settings as ClientSettingsDot).satisfies({
    ...
    fun testAct() {
        ...
        val result = dotMainLoop.act(settings)
